...
[Script based workflows]{.blue-title}
...
In this module we will cover editing and running models using script based workflows using a growth analysis and stock assessment model as examples.
...
- Allows for integration with version control software (e.g., [Git](https://git-scm.com/doc){preview-link="true"}) 
- Facilitates documentation
:::

## Application: growth analysis {.smaller}

</br>

Using a simulated age-and-growth dataset, walk through a simple script based workflow for estimating a [von Bertalanffy growth curve](https://derekogle.com/NCNRS349/modules/PREP/NOTES/Growth){preview-link="true"} in [R](https://www.r-project.org/) using a Lognormal likelihood function.

</br>

::: {.fragment .fade-in}
$$ 
\hat{L} = L_{\infty} (1 - \mathrm{exp}(-K (a - t_0)))
$$
$$
\sigma = \sqrt(\mathrm{log}(1 + CV^2))
$$
$$
\mu = \mathrm{log}(\hat{L}) - \frac{\sigma^2}{2}
$$
$$ 
L_{obs} \sim \mathrm{Lognormal}(\mu,\sigma)
$$
:::

## Fitting the growth model {.smaller}

::: {.codewindow .r .absolute top="15%" left="0%" right="50%" style="font-size: 0.5em;"}
01-00-fit-vb-growth-curve.r
```{.r}
# load packages
    library(data.table)
    library(ggplot2)
...
::: {.absolute top="25%" left="55%" right="0%"}
```{r}
#| eval: true
#| echo: false
#| output: true
...
## Application: stock assessment {.smaller}

</br>

Modify a [Stock Synthesis](https://nmfs-ost.github.io/ss3-website/){.fragment .hl-cur-blue fragment-index=1 preview-link="true"} input model file and running the modified model through [R](https://www.r-project.org/){.fragment .hl-cur-blue fragment-index=2 preview-link="true"} using the [r4ss](https://r4ss.github.io/r4ss/index.html){.fragment .hl-cur-blue fragment-index=3 preview-link="true"} package.

::: {.fragment .fade-out fragment-index=4}
::: {.fragment .fade-in fragment-index=1}
![](static/stock-synthesis.png){.absolute top=32.5% height=35% left=25% style="border: 1px solid #323C46; max-height: unset; max-width: unset; box-shadow: 0 0 2rem 0 rgba(0, 0, 0, .5); border-radius: 5px;"}
:::
:::

::: {.fragment .fade-out fragment-index=4}
::: {.fragment .fade-in fragment-index=2}
![](dynamic/R-logo.svg){.absolute top=32.5% left=5% height=15% style="max-height: unset; max-width: unset;"}
:::
:::

::: {.fragment .fade-out fragment-index=4}
::: {.fragment .fade-in fragment-index=3}
![](static/r4ss-logo.png){.absolute top=50% left=7.5% height=15% style="max-height: unset; max-width: unset;"}
:::
:::


::: {.fragment .fade-up fragment-index=4}
::: {.absolute right="60%" top="35%" left="0%"}
- Specifically, in our example we will run models that make alternative assumptions for the [steepness](https://www.iss-foundation.org/glossary/steepness/){preview-link="true"} ($h$) parameter which controls the shape of the [stock-recruitment relationship](https://edis.ifas.ufl.edu/publication/FA234){preview-link="true"} (SRR).
:::
:::

::: {.fragment .fade-in fragment-index=4}
![](static/srr.png){.absolute top=32.5% left=45% height=50% style="border: 1px solid #323C46; max-height: unset; max-width: unset; box-shadow: 0 0 2rem 0 rgba(0, 0, 0, .5); border-radius: 5px;"}

::: {.absolute left=45% bottom=10% style="font-size:0.4em;"}
[Photo credit:]{.hide-text} <a href="https://mavensnotebook.com/glossary/stock-recruitment-relationship/" class="hiddenlink">Maven's Notebook</a>
:::
:::

## Running Stock Synthesis {.smaller}

Four files (plus the [executable](https://github.com/nmfs-ost/ss3-source-code/releases){preview-link="true"}) are needed to run a Stock Synthesis model:

::: {.absolute left=0% right=75% top=30%}
::: {.fragment .fade-in fragment-index=1}
- `starter.ss` 
:::
:::

::: {.absolute left=0% right=75% top=40%}
::: {.fragment .fade-in fragment-index=2}
- `forecast.ss`
:::
:::

::: {.absolute left=0% right=75% top=50%}
::: {.fragment .fade-in fragment-index=3}
- `data.ss`
:::
:::

::: {.absolute left=0% right=75% top=60%}
::: {.fragment .fade-in fragment-index=4}
- `control.ss` [this is where the SRR is defined]{.fragment .fade-in fragment-index=5}
:::
:::

::: {.fragment .fade-in fragment-index=6}
::: {.absolute left=0% bottom=0% right=75% style="font-size:1em;"}
Check out the [manual](https://nmfs-ost.github.io/ss3-doc/SS330_User_Manual_release.html){preview-link="true"} for further details!
:::
:::

::: {.fragment .fade-in fragment-index=6}
::: {.smaller .absolute top="23%" bottom="2%" left="30%" right="0%"}
```{.r code-line-numbers="|119-127|124"}
...
## Modifying `control.ss` using r4ss {.smaller}

::: {.fragment .fade-out fragment-index=2}
::: {.absolute left=0% top=15% right=75%}
Use [`SS_readctl()`](https://r4ss.github.io/r4ss/reference/SS_readctl.html){preview-link="true"} to read in the control file.
:::
:::

::: {.fragment .fade-out fragment-index=2}
::: {.fragment .fade-in fragment-index=1}
::: {.codewindow .r .absolute top="15%" left="30%" right="0%" style="font-size: 0.6em;"}
01-01-modify-stock-synthesis.r
```{.r}
# load packages
    library(r4ss)
...
::: {.fragment .fade-in fragment-index=2}
::: {.absolute top="15%" left="30%" right="0%"}
```{r}
#| echo: true
class(tmp_ctl)
names(tmp_ctl)
...
::: {.absolute left=0% top=15% right=75%}
Use [`run()`](https://r4ss.github.io/r4ss/reference/run.html){preview-link="true"} to run the model.
...
## Testing multiple alternatives {.smaller}
...
Say also testing $h = 0.5$, $h = 0.6$ and $h = 0.8$? 
:::
:::
...
::: {.fragment .fade-in fragment-index=1}
::: {.codewindow .r .absolute top="15%" left="30%" right="0%" style="font-size: 0.5em;"}
01-02-modify-stock-synthesis-for-loop.r
```{.r}
...
Let's check out what the [models](https://connect.fisheries.noaa.gov/2025-01-ISC-OpenScienceTraining/){preview-link="true"}!
:::
...
Modify the growth analysis code `01-00-fit-vb-growth-curve.r` to estimate sex-specific growth curves. 

 </br>

Each group should set-up their own `branch` on the [GitHub repo](https://github.com/MOshima-PIFSC/ISC_OSworkflow_training) to house the code, model files, and model results for the feature that they explore.
