...
# define paths
	...
	dir_model = file.path(proj_dir,"stock-synthesis-models")
    dir_shiny_data = file.path(proj_dir,"shiny","shiny-data")
    dir.create(dir_shiny_data,recursive=TRUE)
...
# extract output from models
  output = SSgetoutput(dirvec=file.path(dir_model,all_dirs))
  ...
  fwrite(expected_srr_dt,file=file.path(proj_dir,"shiny","shiny-data","expected_srr.csv"))
  ...
  fwrite(est_recruit_dt,file=file.path(proj_dir,"shiny","shiny-data","est_recruit.csv"))
  
#_____________________________________________________________________________________________________________________________
# make summary files to run the shiny app from
  summary_dt = data.table(model_name=names(output),steepness=model_steepness)
  fwrite(summary_dt,file=file.path(proj_dir,"shiny","shiny-data","summary.csv"))
